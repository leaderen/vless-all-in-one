...
db_add_routing_rule() {
    ...
    # custom 类型：插入到数组开头（优先级最高），不删除已有的 custom 规则
    # all 类型：追加到数组末尾（优先级最低）
    # 其他类型：覆盖同类型规则，插入到 custom 规则之后、all 规则之前
    if [[ "$rule_type" == "custom" ]]; then
        # custom 规则插入到开头，优先级最高
        ...
        # 预设规则：删除同类型旧规则，插入到 custom 规则之后
        jq --arg id "$rule_id" --arg type "$rule_type" --arg out "$outbound" --arg domains "$rule_domains" --arg ipv6 "$ipv6_only" \
            '.routing_rules = (
                ((.routing_rules // []) | map(select(.type == "custom"))) + 
                [{id: $id, type: $type, outbound: $out, domains: $domains, ipv6_only: ($ipv6 == "true")}] +
                ((.routing_rules // []) | map(select(.type != "custom" and .type != $type)))
            )' \
            "$DB_FILE" > "$tmp" && mv "$tmp" "$DB_FILE"
    fi
}
